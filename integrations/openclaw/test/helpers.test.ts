...
test("buildAuthorizeUrl produces PKCE-friendly NyxID authorize URL", () => {
  ...
  assert.match(
    url,
    /^https:\/\/nyx-api\.chrono-ai\.fun\/oauth\/authorize\?response_type=code&client_id=client-123/,
  );
...
test("buildProxyUrl normalizes service path", () => {
  assert.equal(
    buildProxyUrl("https://nyx-api.chrono-ai.fun/", "twitter", "/2/tweets"),
    "https://nyx-api.chrono-ai.fun/api/v1/proxy/s/twitter/2/tweets",
  );
});
