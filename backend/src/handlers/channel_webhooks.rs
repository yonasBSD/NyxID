...
pub async fn discord_webhook(
    ...
    let adapter = match resolve_adapter("discord", &state.tenant_token_cache) {
        Ok(a) => a,
...
pub async fn lark_webhook(
    ...
    let adapter = match resolve_adapter("lark", &state.tenant_token_cache) {
        Ok(a) => a,
        Err(_) => return (StatusCode::INTERNAL_SERVER_ERROR, "".to_string()).into_response(),
    };
...
pub async fn feishu_webhook(
    ...
    let adapter = match resolve_adapter("feishu", &state.tenant_token_cache) {
        Ok(a) => a,
        Err(_) => return (StatusCode::INTERNAL_SERVER_ERROR, "".to_string()).into_response(),
    };
...
async fn handle_webhook_inner(
    ...
    let adapter = resolve_adapter(&bot.platform, &state.tenant_token_cache).map_err(
